...
### Package rebuilt after the popup visual rebuild (2026-09-17, delivery packaging)

The `ff76a239…` hash above describes the package as it stood when Gate D closed. Commit `229b483` then rebuilt the popup stylesheet and added the bundled fonts, and the packaged ZIP was not rebuilt with it, so the recorded hash no longer described the source tree. The package was rebuilt from the current `extension/dist` while assembling the v0.1.0 delivery bundle.

- 25 files, up from 18. `popup.css` is 26 505 bytes, up from 4 198, and the `fonts/` directory is now included.
- `unzip -t` reported no errors, and the contents are identical to the freshly built `dist`.
- New SHA-256: `30e9c21b9e2f1c27c9aa0f6c6f73724a799c7e42549fcb38b4396226cefbdb5e`.
- No source file was changed by this rebuild.

The `ff76a239…` hash stays recorded above because it is the hash that was verified at Gate D; it does not describe the package as it now stands.
