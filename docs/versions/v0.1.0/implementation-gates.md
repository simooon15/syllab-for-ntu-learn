...
- All remaining Phase 13–15 real-environment work is consolidated into the single procedure in `gate-d-consolidated-acceptance.md`. Existing evidence is referenced and is not rerun unless a later code change invalidates it.
...
The `ff76a239…` value above is the package verified when Gate D closed. After the popup visual rebuild, the current delivery package was rebuilt from `extension/dist` with 25 files, bundled fonts and SHA-256 `30e9c21b9e2f1c27c9aa0f6c6f73724a799c7e42549fcb38b4396226cefbdb5e`. The old value remains as historical Gate D evidence; see `final-acceptance-v0.1.0.md` for the package lineage.
