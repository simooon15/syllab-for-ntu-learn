...
![侧栏课程列表](docs/versions/v0.2.0/images/SEM-02.png)
...
![首次扫描](docs/versions/v0.2.0/images/ISC-02.png)
...
![Change Review](docs/versions/v0.2.0/images/CRV-01.png)
...
![备份 / 恢复](docs/versions/v0.2.0/images/BKP-01.png)
...
**下载 →** [`artifacts/Syllab_Extension_v0.2.0.zip`](artifacts/Syllab_Extension_v0.2.0.zip)
...
7. **按提示填入你自己的 DeepSeek API Key**（Settings → AI）。Key 只保存在本机，会上传到的地方
   只有 DeepSeek 本身；它不会进入备份文件，也不会进入本仓库。
...
> `npm run gate:2` 每次都会重新打包这个 zip，并用全新浏览器 profile 实际加载验证它可用。
...
**v0.2.0 — 开发完成，等待最终 Human / Product Gate 验收。**

v0.2.0 把产品从“能够生成 Course Brief 的单课程 MVP”推进为“围绕一个 Semester 持续维护课程
状态的完整个人课程信息工具”，并收口了 v0.1.0 遗留的 KR-07、KR-08 与 E3。

验收证据与已知限制以
[docs/versions/v0.2.0/](docs/versions/v0.2.0/) 与 `artifacts/gates/v0.2.0/` 下的 Gate 报告为准。
...
v0.2.0 功能开发与自动化验收已完成，等待最终 Human / Product Gate 验收，因此还不是正式生产
发布版本。
...
![Side Panel Course List](docs/versions/v0.2.0/images/SEM-02.png)
...
![Initial Scan](docs/versions/v0.2.0/images/ISC-02.png)
...
![Backup / Restore](docs/versions/v0.2.0/images/BKP-01.png)
...
**Download →** [`artifacts/Syllab_Extension_v0.2.0.zip`](artifacts/Syllab_Extension_v0.2.0.zip)
...
7. **Add your own DeepSeek API key when asked** (Settings → AI). The key is stored on this machine
   only; the only place it is ever sent is DeepSeek itself. It never enters a backup and never enters
   this repository.
...
> "Load unpacked". `npm run gate:2` rebuilds this zip every run and loads it in a clean browser
> profile to prove it works.
...
**v0.2.0 — development complete, awaiting final Human / Product Gate acceptance.**
...
Acceptance evidence and known limitations are the Gate reports under
[docs/versions/v0.2.0/](docs/versions/v0.2.0/) and `artifacts/gates/v0.2.0/`.
...
v0.2.0 development and automated acceptance are complete; the version is awaiting final Human /
Product Gate acceptance and is not a production release.
